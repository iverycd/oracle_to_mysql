...
def tbl_columns(table_name, fix_mode='N'):
    oracle_cursor = configDB.OraclePool()  # Oracle连接池
    col_len = 0
    output_table_col = []
    default_str = ''
    sql = """SELECT A.COLUMN_NAME, A.DATA_TYPE, A.CHAR_LENGTH, case when A.DATA_PRECISION is null then -1 else  A.DATA_PRECISION end DATA_PRECISION, case when A.DATA_SCALE is null then -1 else  A.DATA_SCALE end DATA_SCALE,  case when A.NULLABLE ='Y' THEN 'True' ELSE 'False' END as isnull, B.COMMENTS,A.DATA_DEFAULT,case when a.AVG_COL_LEN is null then -1 else a.AVG_COL_LEN end AVG_COL_LEN
...
# 打印连接信息
def get_info(run_method, mode, log_path):
    oracle_cursor = configDB.OraclePool()  # Oracle连接池
    ...
    k.add_row(["Tool Version: " + '1.9.15.2'])
    k.add_row(["Release Date: " + '2022-09-15'])
    ...
    x.add_row(['Oracle', str(oracle_info['user']),
               str(oracle_info['dsn']).replace('DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)', '').replace('CONNECT_DATA=', '')])
    # 表排序
    ...
    if run_method == 1:
        ...
        x = prettytable.PrettyTable(field_names=["tables", "views", "triggers", "procedures", "functions", "packages"])
        x.align["tables"] = "l"  # 以序号字段左对齐
        x.padding_width = 1  # 填充宽度
        x.add_row(
            [str(source_table_count), str(source_view_count), str(source_trigger_count), str(source_procedure_count),
    ...
    x.align["database"] = "l"  # 以序号字段左对齐
    x.padding_width = 1  # 填充宽度
    x.add_row(['MySQL', str(mysql_info['host']), str(mysql_info['port']), str(mysql_info['user']),
               str(mysql_info['database'])])
    # 表排序
    print(x.get_string(sortby="database", reversesort=False))
    ...
    mysql_cursor.execute("""create table my_mig_task_info(table_name varchar(500),task_start_time datetime(3) default current_timestamp(3),
...
def cte_tab(log_path, is_custom_table):
    ...
    if is_custom_table == 1:
        ...
        drop_table_name = oracle_cursor.fetch_all(tableoutput_sql)
    ...
    for row in output_table_name:
        ...
        try:
            ...
            if 'Row size too large' in str(e):
                ...
                fieldinfos = []
                structs = tbl_columns(table_name, 'FIX')  # 使用fix方式获取源表实际列长度拼接列字段信息
                for struct in structs:
                    defaultvalue = struct.get('default')
                    commentvalue = struct.get('comment')
                    if defaultvalue:  # 对默认值以及注释数据类型的判断，如果不是str类型，转为str类型
...
# 批量创建主键以及索引
def cte_idx(log_path, is_custom_table):
    oracle_cursor = configDB.OraclePool()  # Oracle连接池
    mysql_cursor = configDB.MySQLPOOL.connection().cursor()  # MySQL连接池
    ...
    if all_constraints_count > 0:
        ...
        for d in all_index:
            ...
            try:
                ...
                # print('FINISH CREATE NORMAL INDEX\n')
...
# 查找具有自增特性的表以及字段名称
def cte_trg(log_path, is_custom_table):
    oracle_cursor = configDB.OraclePool()  # Oracle连接池
    mysql_cursor = configDB.MySQLPOOL.connection().cursor()  # MySQL连接池
    ...
    if is_custom_table == 1:  # 如果命令行参数有-c选项，仅创建部分自增列
        with open(log_path + "table.txt", "r") as f:  # 读取自定义表
            for table_name in f.readlines():  # 按顺序读取每一个表
                ...
                try:
                    oracle_cursor.execute_sql("""update trigger_name set trigger_body=upper(trigger_body) """)
                    oracle_cursor.execute_sql(
        ...
        try:
            oracle_cursor.execute_sql("""update trigger_name set trigger_body=upper(trigger_body) """)
            oracle_cursor.execute_sql("""update trigger_name set trigger_body=replace(trigger_body,'INTO:','INTO :')""")
            oracle_cursor.execute_sql(
                """update trigger_name set trigger_body=replace(trigger_body,'SYS.DUAL ','DUAL')""")
            oracle_cursor.execute_sql(
                """update trigger_name set trigger_body=replace(trigger_body,'SYS.DUAL','DUAL')""")
...
def cte_comt(log_path, is_custom_table):
    oracle_cursor = configDB.OraclePool()  # Oracle连接池
    mysql_cursor = configDB.MySQLPOOL.connection().cursor()  # MySQL连接池
    err_count = 0
    all_comment_sql = []
...
def cp_vw():
    oracle_cursor = configDB.OraclePool()  # Oracle连接池
    all_view_compile = oracle_cursor.fetch_all("""select 'alter view '||view_name||' compile' from user_views""")
    for exe_compile in all_view_compile:
        exe_compile_sql = exe_compile[0]
        print(exe_compile_sql)
        try:
            oracle_cursor.execute_sql(exe_compile_sql)
        except Exception as e:
            print(e)


# 获取视图定义以及创建
def c_vw(log_path, is_custom_table):
    oracle_cursor = configDB.OraclePool()  # Oracle连接池
    mysql_cursor = configDB.MySQLPOOL.connection().cursor()  # MySQL连接池
    all_view_count = 0
    view_failed_result = []
